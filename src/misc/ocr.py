...
from pytesseract import pytesseract, Output, TesseractNotFoundError
...
def ocr_rect(page: PdfPage, rect: pymupdf.Rect, language='swe') -> str|None:
    """See language support here https://tesseract-ocr.github.io/tessdoc/Data-Files-in-different-versions.html"""
    pixmap = page.page().get_pixmap(matrix=MATRIX, clip=rect, alpha=False)
    img = Image.open(io.BytesIO(pixmap.tobytes('png')))
    try:
        return pytesseract.image_to_string(img, lang=language, config=f'--psm {PSM_WORD}', output_type=Output.STRING).strip()
    except (RuntimeError, TesseractNotFoundError) as e:
