...
3. Run it. Note the install path — default is usually `C:\Program Files\Tesseract-OCR`
   1During install, there's a component list — make sure "Additional language data" is checked if you need anything beyond English
4. Add the install path (Probably `C:\Program Files\Tesseract-OCR`) to the PATH variable.
5. Test that Tesseract was successfully added to PATH by running `tesseract --version` in a terminal.
